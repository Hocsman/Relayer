              ...
              {settings.webhooks.map((hook, index) => (
                <tr key={`${index}-${hook.name}`}>
                  ...
                  <td style={{ maxWidth: "240px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={hook.url}>
                    {hook.url}
                    {hook.hasHeaders && (
                      <span className="settings-webhook-headers" title="This webhook has headers (for example an Authorization credential). Their values are not shown. Saving keeps them unless you change this webhook's URL, rename it while another uses the same URL, or add or remove an identical webhook.">
                        {" "}🔒 headers kept
                      </span>
                    )}
                  </td>
