...
func TestGetAgentProfilesReturnsSafeCatalogAndLocksAdvancedSpecs(t *testing.T) {
	...
	if len(view.Catalog) != 8 || view.Catalog[0].ID != string(toolcatalog.ClaudeCode) || !view.Catalog[0].Installed {
		t.Fatalf("catalog = %#v", view.Catalog)
	}
	if view.Catalog[1].Installed || view.Catalog[7].InstallStatus != string(toolcatalog.InstallUnknown) {
		t.Fatalf("catalog installation states = %#v", view.Catalog)
	}
	if got := view.Catalog[6].DefaultArgv; len(got) != 3 || got[0] != "ollama" || got[1] != "run" || got[2] != "" {
		t.Fatalf("Ollama default argv = %#v", got)
	}
	if got := view.Catalog[6].ArgumentPrefix; len(got) != 1 || got[0] != "run" {
	...
	view.Catalog[6].ArgumentPrefix[0] = "mutated"
	...
	if fresh.Profiles[0].Name != "Claude Code" || fresh.Catalog[0].DefaultArgv[0] != "claude" ||
		fresh.Catalog[6].ArgumentPrefix[0] != "run" {
