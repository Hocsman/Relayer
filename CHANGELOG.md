...
- **"Save and restart" could keep changes it said it had undone**: when the security or notification tab had changed, the panel saved them through a separate call before starting the restart transaction, so the transaction took its rollback snapshot of a file those tabs had already changed. A failed restart then kept them, and the panel said "the previous YAML was restored". Every tab now travels in the one transactional request, written after the snapshot, and on the desktop a failed restart restores the file byte for byte. The web gateway, which has no rollback, now writes every tab in the same atomic write as the agents; a failed restart there leaves that file in place.
  - After a failure the panel reloads every tab from the file, not only the agents, and says it did only if the reload succeeded. It used to say so either way, and a form still showing the unsaved changes looked like the file.
- **Every settings save erased every webhook's headers**: the editor never receives header values — they are usually an `Authorization` credential — so each webhook it sent back had none, and the save replaced the whole notifications block with that. On the desktop the notifier was rebuilt at once, so the next alert went out unauthenticated. Headers are now carried through a save by the webhook's URL and name, or by its URL alone when only one webhook has that URL. A webhook moved to a new URL does not take the old endpoint's credential, and neither does a webhook without headers that shares its URL with one that has them. The editor shows that a webhook has headers, never their values.
  - A webhook that could not be delivered left no trace. `relayer serve` now writes one line per failure to its diagnostics, and the desktop to its standard error, with the webhook's name and the HTTP status and never its URL — for Slack and Discord the URL is the credential — nor the transport error, which quotes it. The TUI still reports none: it owns the terminal.
...
- **A viewer token still received every agent's command line, credentials included** — v0.8.4 said viewers were given agent configurations "without exposing secrets". `getAgentProfiles` returned each agent's full argument vector and the configuration's path; a token planted in an agent's `--api-key` came back verbatim to a viewer. A viewer now gets each agent's executable name and argument count, and no configuration path. Operators, who can already rewrite and restart agents, are unchanged; the desktop never sends argument vectors to its interface at all.
  - A viewer also no longer receives the configuration's or the audit journal's path. `getState`'s startup notices named both, and the audit summary and the journal check returned the journal's path; an error that would quote one reaches a viewer as a fixed message. `getState`'s agent list was already safe: its display command is only the executable's name, and viewers keep it.
...
- **An agent that exited on its own could only be restarted once** — v0.8.5 announced the fix, but it held for a single cycle and only on the desktop.
  ...
  - Web gateway: an exit was broadcast as `stopped`, a status the interface does not clear "running" on, so the card kept a Stop button that could only fail; a start never set the agent back to running, so the card offered Start for a live agent. Exits are now `exited` or `failed`, a lost tmux session is `failed` as on the desktop, and a start marks the agent running and tells every client.
  ...
  - An exit names only the agent, and the previous process's exit can be emitted after a replacement started. It marked the replacement stopped in the lifecycle, which let a second Start run beside it. The lifecycle now applies an exit only when the backend reports the current process gone. The desktop then journals the exit as the previous process's and keeps the agent running on screen; the web gateway ignores it; the TUI still shows the pane exited.
...
- `docs/web-gateway.md` gives the nginx `Host` line that keeps the port, says a tokenless gateway refuses tunnels and port forwards, that `--bind localhost` is anonymous like `127.0.0.1`, and who receives agent command lines. `docs/architecture.md` no longer says Relayer never signals a process by a reusable number on Unix, where the cleanup right after the reap still addresses the group by number, and it says the graceful stop covers PTY agents and not tmux.
