...
- **A security save rewrote the whole `policies` block**, on the desktop and the web gateway alike, since the settings editor was introduced in v0.3.0: the block was rebuilt from the effective policy, which drops what that rebuild does not write. A save that only toggled dry-run, or changed nothing, removed `profile: custom` and every guardrail set to `false`, and lost the block's comments and flow lists; since v0.8.6, which resolves a relative `workspace_root` when loading, it also wrote `workspace_root: ./workspace` back as an absolute path naming the user's home directory. The block is now edited: a field whose value the file already gives keeps its text, a changed field is written where it is, a profile the file names stays as the base of the fields it does not set, and a save that changes nothing writes nothing. An absolute or rooted `workspace_root` keeps its text too: the loader now cleans it as the save does, so a save no longer rewrites `/srv/ws` as `\srv\ws` on Windows, which Linux does not read as that directory, so that with a configuration shared with Linux every path an agent named there was outside the workspace, or drops a trailing separator (`TestASecuritySaveKeepsAnAbsoluteWorkspaceRootAsWritten`). Turning the workspace guardrail on in a file that names no root writes only the guardrail: the save also wrote the root the guardrail defaults to, the configuration's directory, as an absolute `workspace_root`, which then no longer followed the file when it moved (`TestTurningTheWorkspaceGuardrailOnWritesOnlyTheGuardrail`). When the editor switches to a preset and the file names a profile, the file names that preset, also when a field was adjusted after the preset was chosen: the writer recognized only a preset the policy matched exactly, so choosing `strict` and then setting the rate limit dropped the `profile` line and its comments and wrote every field of `strict` out (`TestAPresetChosenThenAdjustedIsNamedWhereTheFileNamesAProfile`, `TestAWebPresetChosenThenAdjustedIsNamedInTheFile`, `TestADesktopPresetChosenThenAdjustedIsNamedInTheFile`). Switching from `strict` to `permissive` in a file that names no root keeps it relative: `strict` guards the configuration's directory, `permissive` turns the guardrail off and the editor keeps the root it shows, so the save named that root, as an absolute path. It now writes `workspace_root: .`, which follows the file as the implicit root did, and so do turning the guardrail off in a file that spells `strict` out and choosing a preset in a file without a `policies` block, which is written whole (`TestSwitchingFromStrictToPermissiveKeepsTheWorkspaceRootRelative`, `TestAPoliciesBlockWrittenWholeKeepsTheWorkspaceRootRelative`, `TestAWebSwitchFromStrictToPermissiveKeepsTheWorkspaceRootRelative`, `TestADesktopSwitchFromStrictToPermissiveKeepsTheWorkspaceRootRelative`; on a file with Windows line endings and a byte-order mark, followed by a notifications save that changes nothing, `TestAWebSwitchToPermissiveThenAnUnchangedNotificationsSaveKeepAWindowsFileAsWritten` and its desktop counterpart, which CI runs on Windows). The writer refuses to publish a file whose policy would not load back as requested. `TestASecuritySaveChangesOnlyWhatItChanged`, `TestAWebSecuritySaveKeepsThePolicyAsWritten` and `TestADesktopSecuritySaveKeepsThePolicyAsWritten` fail against v0.8.9. Found by the v0.8.9 acceptance pass; v0.8.8 did the same.
- **A notifications save rewrote the whole `notifications` block**, on the desktop and the web gateway alike: a save that changed one field, or sent the tab back unchanged, rebuilt the block from the editor's form. It lost its comments and the flow style of a webhook's headers, and every field the file leaves to its default was written out, down to a webhook's empty `name`, `min_severity` and `timeout`. The block is now edited as the agents are. Notifications that behave as the file's write nothing, and the revision stays: a webhook without `format`, `min_severity` or `timeout` is the `generic`, `warning`, `5s` one the desktop's editor sends back for it, and `format: Slack` is the `slack` it sends. A changed field is written where it is, or added to its section; a webhook the save did not change keeps its entry as written, and a changed one, or one renamed at a URL no other webhook uses, keeps every field it did not change; a webhook added in the place of a removed one is a new entry and takes nothing from it. The writer refuses to publish a file whose notifications would not load back as requested. `TestANotificationsSaveThatChangesNothingWritesNothing`, `TestASettingsSaveKeepsTheNotificationsItDidNotChange`, `TestANotificationsSaveChangesOnlyWhatItChanged`, `TestAWebNotificationsSaveKeepsTheBlockAsWritten` and `TestADesktopNotificationsSaveKeepsTheBlockAsWritten` fail without this fix.
- **A click on a settings tab did nothing**, on the desktop and in the web interface alike: the dialog's grid had three rows for its four parts, so the tab bar got no height and the Agents tab's content covered it, and "Security & Guardrails" and "Notifications & Webhooks" could be reached only from the keyboard. Each part of the dialog now has its own row, the banner shown to a viewer included. The stylesheet's own tests had never run: Vitest hands a test an empty string for a CSS import unless told otherwise, so `styles.test.ts` parsed nothing. They now read the stylesheet, and the check they already had, that no rule removes the focus outline outside a `:focus` rule, found three: the settings' drop-downs had no focus ring, and now have the one buttons and fields have. `e2e/supervisor.spec.ts` clicks through the tabs, and a stylesheet test fails without this fix.
...
- **A journal failed its own verification once a person answered a sensitive prompt**, in the `detailed` audit mode: the sanitizer gave a person's decision on a password prompt, or on any high-risk one such as Aider's "Run shell command?", the constant summary of a sensitive entry, and `relayer audit verify`, the interface's Verify with it, rejects any summary on a person's decision. A person's decisions carry none now, and the verifier accepts the constant, which is no free text, so journals written before this release verify (`TestAPersonsDecisionOnASensitivePromptCarriesNoSummary`, `TestAJournalWithThePersonsSensitiveConstantVerifies`). Found by the acceptance pass; v0.8.8 did the same.
- **Stopping `relayer serve` let a held terminal go as a disconnect**: on Ctrl+C the gateway shut its server down, closing every WebSocket, before it ended the run, so a held terminal was journaled `control_released_disconnect` and clients never saw the run end. The run now ends first, while its clients are connected (`TestStoppingServeLetsAHeldTerminalGoWithTheRun`).
...
- **A prompt shown while its holder typed is the terminal's**, and any keystroke counts, a mouse report included: clicking into the terminal of an agent that asked for mouse reports is enough. The terminal's own replies do not count: a focus report, a cursor position report or the terminal's identification is written to the agent but leaves the prompts answerable. The ConPTY of every Windows session asks for focus reports in its first bytes, so taking a terminal there always sent one (`TestAWebTerminalsFocusReportLeavesThePromptAnswerable`).
...
- **A prompt that offers Deny alone takes no typed answer**, on the desktop as on the gateway: typed text is whatever the adapter reads it as, its accept included. The generic and Claude adapters encode no deny, so a prompt of theirs that the policy would deny, as a deny rule or `default_action: deny` does for a valid non-sensitive confirmation, offers no button at all; there a typed answer is the only one there is, and it is taken, as before, with the person's `decision` entry journaled under the reason `typed_over_policy_deny`, whatever they typed (`TestATypedAnswerIsTakenWhereTheAdapterEncodesNoDeny`). The TUI still asks it without restriction.
...
- **The journal**: a person's `decision` and `delivery` on the gateway carry the connection (`conn_id`) beside the operator and role, as metadata that only the `detailed` mode keeps; the default `metadata` mode keeps the `operator` field alone. A line's two `operator_input` entries name the operator and nothing else. The policy's `decision` entries carry the rule that made them; a person's `decision` and `delivery` on the gateway no longer do, as on the desktop: the rule is on the prompt's `policy_evaluated` entry. The gateway's lines are journaled before they are written as well as after.
