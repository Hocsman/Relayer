...
- **"Save and restart" could keep changes it said it had undone**: when the security or notification tab had changed, the panel saved them through a separate call before starting the restart transaction, so the transaction took its rollback snapshot of a file those tabs had already changed. A failed restart then kept them, and the panel said "the previous YAML was restored". Every tab now travels in the one transactional request, written after the snapshot, and a failed restart restores the file byte for byte; the web gateway writes them in the same atomic write as the agents. After a failure the panel reloads every tab from the file, not only the agents.
- **Every settings save erased every webhook's headers**: the editor never receives header values — they are usually an `Authorization` credential — so each webhook it sent back had none, and the save replaced the whole notifications block with that. On the desktop the notifier was rebuilt at once, so the next alert went out unauthenticated. Headers are now carried through a save by the webhook's URL and name, or its URL alone when unambiguous; a webhook moved to a new URL does not take the old endpoint's credential. The editor shows that a webhook has headers, never their values.
  - A webhook that could not be delivered left no trace. The web gateway now writes one line per failure to its diagnostics, with the webhook's name and the HTTP status, and never its URL — for Slack and Discord the URL is the credential — nor the transport error, which quotes it.
...
- **A viewer token still received every agent's command line, credentials included** — v0.8.4 said viewers were given agent configurations "without exposing secrets". `getAgentProfiles` returned each agent's full argument vector and the configuration's path, and `getState` and `/api/state` returned the full display command; a token planted in an agent's `--api-key` came back verbatim to a viewer. A viewer now gets each agent's executable name and argument count, and no configuration path. Operators, who can already rewrite and restart agents, are unchanged; the desktop never sends argument vectors to its interface at all.
  ...
  - `TestViewerNeverReceivesAnAgentsArguments` plants a credential in an agent's argv and checks all three paths; it fails against v0.8.5. The recordings test that should have caught a viewer deleting a transcript rendered before any recording existed, and passed with the guard removed; it now checks the rule and the rendered rows.
...
- **Windows: every Stop waited out a grace period meant for Unix**: v0.8.5 applied the 1.5-second SIGTERM grace on Windows too, where the polite request reaches nothing, so each Stop and Restart took about 1.7 s instead of 0.2 s. Closing the pseudo console — which sends `CTRL_CLOSE_EVENT` to every attached process — is the graceful stop there, and it now happens immediately.
- **Unix: shutting down still killed every agent with SIGKILL first**: v0.8.5 gave a single `Stop` a SIGTERM grace period, but `Manager.Close` cancelled its context before stopping anything, and an agent's command is an `exec.CommandContext` whose default cancellation is an immediate SIGKILL of the leader. The same happened on every path that cancels a parent context first: the desktop and TUI shutting down, a signal to `relayer serve`, a web "Save and restart".
...
- **The desktop announced policy changes as "applied immediately" while nothing enforced them**: the policy engine is built once per run and has no way to swap its configuration, but the desktop's save treated a run as current whenever the agents were unchanged. After switching on dry-run, the top bar showed DRY RUN and the panel said the change was applied, while automatic approvals kept being delivered by the old engine. v0.8.5 fixed this for the web gateway only, and its test for the desktop asserted the wrong behaviour.
  - Both front ends now report a restart as required whenever a save changes the agents or the policy, and never report one for a save that only changes notifications, which both apply at once. The web gateway used to ask for a restart after a notification-only save.
  - The desktop top bar keeps showing the policy the running engine enforces, not the one just saved.
...
- **An agent that exited on its own could only be restarted once** — v0.8.5 announced the fix, but it held for a single cycle and only on the desktop.
  - Desktop: a fresh process numbers its events from 1 again and event IDs derive from that sequence, so the second process's exit carried the same ID as the first one's. The desktop kept that ID as already handled and dropped the second exit; the agent then looked running forever and Start was refused as "still running". A start now forgets the previous process's handled events.
  - Web gateway: an exit was broadcast as `stopped`, a status the interface does not clear "running" on, so the card kept a Stop button that could only fail; a start never set the agent back to running, so the card offered Start for a live agent. Exits are now `exited` or `failed`, and a start marks the agent running and tells every client.
  - TUI: exits never reached the lifecycle, so restarting a dead agent first "stopped" it and journaled a second `session_finished`. The TUI now reports exits, off its update loop, and a restart no longer stops a process the backend reports gone.
  - An exit names only the agent, and the previous process's exit can be emitted after a replacement started. It marked the replacement stopped, which let a second Start run beside it. An exit is now applied only when the backend reports the current process gone; otherwise it is journaled as the previous process's and changes nothing on screen.
  - `TestRestartAfterASecondNaturalExit` and `TestWebAgentCanBeStoppedAndStartedAgainAndAgain` run several cycles on the real paths; both fail against v0.8.5.
