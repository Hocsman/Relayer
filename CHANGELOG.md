...
- **A notifications save rewrote the whole `notifications` block**, on the desktop and the web gateway alike: a save that changed one field, or sent the tab back unchanged, rebuilt the block from the editor's form. It lost its comments and the flow style of a webhook's headers, and every field the file leaves to its default was written out, down to a webhook's empty `name`, `min_severity` and `timeout`. The block is now edited as the agents have been since v0.8.10. Notifications that behave as the file's write nothing, and the revision stays: a webhook without `format`, `min_severity` or `timeout` is the `generic`, `warning`, `5s` one the desktop's editor sends back for it. A changed field is written where it is, or added to its section; a webhook the save did not change keeps its entry as written, and a changed one keeps every field it did not change. The writer refuses to publish a file whose notifications would not load back as requested. `TestANotificationsSaveThatChangesNothingWritesNothing`, `TestASettingsSaveKeepsTheNotificationsItDidNotChange` and `TestANotificationsSaveChangesOnlyWhatItChanged` fail against v0.8.10.
