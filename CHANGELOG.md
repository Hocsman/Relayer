...
- **An agent that exited on its own could only be restarted once** — v0.8.5 announced the fix, but it held for a single cycle and only on the desktop.
  ...
  - Web gateway: an exit was broadcast as `stopped`, a status the interface does not clear "running" on, so the card kept a Stop button that could only fail; a start never set the agent back to running, so the card offered Start for a live agent. Exits are now `exited` or `failed`, a lost tmux session is `failed` as on the desktop, and a start marks the agent running and tells every client. An agent that ended loses its pending prompts, as on the desktop, and neither answering one, its late withdrawal, nor an exit right after an answer marks a dead agent running again.
...
- `docs/web-gateway.md` gives the nginx `Host` line that keeps the port, and says which tunnels, forwards and proxies a tokenless gateway refuses and which it accepts anonymously, that `--bind localhost` is anonymous like `127.0.0.1`, who receives agent command lines, and that a container needs an init process (`docker run --init`) or every Stop is reported unconfirmed. `docs/architecture.md` no longer calls the audit journal cryptographic. `docs/architecture.md` no longer says Relayer never signals a process by a reusable number on Unix, where the cleanup right after the reap still addresses the group by number, and it says the graceful stop covers PTY agents and not tmux.
